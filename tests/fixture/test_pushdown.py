...
class TestPruningIsObservable:
    ...
    def test_the_wide_table_reports_columns_scanned_of_columns_available(
        ...
        df = spark.table("fx.wide").select("id", "col_001")
        assert "columns: 2 of 200" in explain_text(df)

    def test_the_partitioned_table_reports_files_scanned_of_files_available(
        self, spark: SparkSession
    ) -> None:
        df = spark.table("fx.partitioned").filter(F.col("as_at_date") == "2026-08-16")
        assert "1 of 3 file(s)" in explain_text(df)

    def test_the_pushed_predicate_is_named_readably(self, spark: SparkSession) -> None:
        df = spark.table("fx.partitioned").filter(F.col("as_at_date") == "2026-08-16")
        assert "pushed filters: as_at_date = '2026-08-16'" in explain_text(df)

    def test_extended_mode_shows_the_optimized_tree_and_the_rules(
        self, spark: SparkSession
    ) -> None:
        df = spark.table("fx.wide").select("id").filter(F.col("id") < 10)
    ...
    def test_an_untranslatable_filter_is_reported_rather_than_hidden(
        ...
        df = spark.table("fx.plain").filter(F.expr("upper(vendor) = 'A'"))
        assert "kept in SQL only:" in explain_text(df)


class TestProjectionPushdown:
    def test_only_the_referenced_columns_reach_read_parquet(self, spark: SparkSession) -> None:
        """3.6: never `SELECT *` on the 200-column table."""
        df = spark.table("fx.wide").select("id", "col_001")
        assert scan_of(df).columns == ("id", "col_001")

    def test_a_column_used_only_in_a_filter_is_still_read(self, spark: SparkSession) -> None:
        df = spark.table("fx.wide").filter(F.col("col_002") > 0).select("id")
        assert set(scan_of(df).columns) == {"id", "col_002"}

    def test_count_star_reads_one_column_not_two_hundred(self, spark: SparkSession) -> None:
        """`count(*)` references no column at all; reading one is the cheapest way to
        get the row count and still have a legal projection."""
        df = spark.table("fx.wide")
        assert df.count() == 500

    def test_an_order_by_on_an_output_alias_still_prunes(self, spark: SparkSession) -> None:
        ...
        df = spark.sql("SELECT id, count(*) AS n FROM fx.wide GROUP BY id ORDER BY id")
    ...
    def test_selecting_everything_reads_everything(self, spark: SparkSession) -> None:
        df = spark.table("fx.wide")
        assert len(scan_of(df).columns) == 200

    def test_the_rows_are_unchanged_by_pruning(self, spark: SparkSession) -> None:
        rows = spark.table("fx.wide").select("id", "col_001").filter(F.col("id") < 3).collect()
        assert sorted(row["id"] for row in rows) == [0, 1, 2]
        assert {row["col_001"] for row in rows} == {2.0}


class TestPredicatePushdown:
    def test_a_partition_predicate_prunes_files(self, spark: SparkSession) -> None:
        df = spark.table("fx.partitioned").filter(F.col("as_at_date") == "2026-08-16")
        assert scan_of(df).files_scanned == 1
        assert scan_of(df).files_total == 3

    def test_the_pruned_query_returns_the_right_rows(self, spark: SparkSession) -> None:
        rows = spark.table("fx.partitioned").filter(F.col("as_at_date") == "2026-08-16").collect()
        assert len(rows) == 4
        assert {row["as_at_date"] for row in rows} == {"2026-08-16"}

    def test_the_filter_is_always_kept_in_the_sql(self, spark: SparkSession) -> None:
    ...
    def test_an_untranslatable_filter_still_filters(self, spark: SparkSession) -> None:
        df = spark.table("fx.plain").filter(F.expr("upper(vendor) = 'A'"))
        assert scan_of(df).pushed_filter is None
        assert sorted(row["id"] for row in df.collect()) == [1, 3]

    def test_a_partial_and_pushes_the_half_it_understands(self, spark: SparkSession) -> None:
        df = spark.table("fx.partitioned").filter(
    ...
    def test_a_predicate_matching_no_partition_reads_no_files(self, spark: SparkSession) -> None:
        df = spark.table("fx.partitioned").filter(F.col("as_at_date") == "1999-01-01")
    ...
    def test_a_partition_column_reads_its_real_value_not_the_directory_name(
        ...
        rows = spark.table("fx.partitioned").collect()
        assert {type(row["as_at_date"]) for row in rows} == {str}
        assert "2026-08-16" in {row["as_at_date"] for row in rows}

    def test_sql_and_the_dataframe_api_prune_identically(self, spark: SparkSession) -> None:
        """P1: one IR, so both surfaces get the same plan and the same pruning."""
        from_api = spark.table("fx.partitioned").filter(F.col("as_at_date") == "2026-08-16")
        from_sql = spark.sql("SELECT * FROM fx.partitioned WHERE as_at_date = '2026-08-16'")
...
class TestRenamedColumns:
    ...
    def test_the_renamed_column_reads_correctly(self, spark: SparkSession) -> None:
        values = [row["new_name"] for row in spark.table("fx.renamed").collect()]
        assert sorted(values) == ["after-c", "after-d", "before-a", "before-b"]

    def test_no_value_comes_back_null(self, spark: SparkSession) -> None:
        """The specific shape of the bug: silence, not an error."""
        values = [row["new_name"] for row in spark.table("fx.renamed").collect()]
        assert None not in values

    def test_it_agrees_with_pyiceberg_which_reads_by_field_id(
        self, spark: SparkSession, fixtures: dict[str, object]
    ) -> None:
        table = fixtures["renamed"].table  # type: ignore[attr-defined]
        expected = table.scan().to_arrow().column("new_name").to_pylist()
        actual = [row["new_name"] for row in spark.table("fx.renamed").collect()]
        assert sorted(actual) == sorted(expected)

    def test_the_files_are_grouped_by_the_names_they_hold(self, spark: SparkSession) -> None:
        scan = scan_of(spark.table("fx.renamed"))
        assert scan.renamed_columns == ("new_name",)
        assert len(scan.groups) == 2, "one group per stored spelling"

    def test_the_reconciliation_is_reported(self, spark: SparkSession) -> None:
        assert "field-id reconciliation: new_name" in explain_text(spark.table("fx.renamed"))

    def test_a_table_that_never_renamed_takes_the_fast_path(self, spark: SparkSession) -> None:
        """Detection is O(schemas), so it must cost nothing on ordinary tables."""
        scan = scan_of(spark.table("fx.plain"))
        assert scan.renamed_columns == ()
        assert len(scan.groups) == 1

    def test_filtering_on_a_renamed_column_still_works(self, spark: SparkSession) -> None:
        rows = spark.table("fx.renamed").filter(F.col("new_name") == "before-a").collect()
...
class TestCopyOnWriteInvariant:
    ...
    def test_reading_a_table_with_delete_files_is_refused(self, spark: SparkSession) -> None:
        with pytest.raises(UnsupportedFeatureError, match="merge-on-read"):
            spark.table("fx.mor").collect()

    def test_the_refusal_says_which_table_and_how_many_files(self, spark: SparkSession) -> None:
        with pytest.raises(UnsupportedFeatureError, match=r"fx\.mor.*1 data file"):
            spark.table("fx.mor").count()
    ...
    def test_it_is_refused_rather_than_silently_answered_wrongly(
        ...
        with pytest.raises(UnsupportedFeatureError):
            spark.table("fx.mor").toArrow()

    def test_planning_is_refused_too_not_just_execution(self, spark: SparkSession) -> None:
        """`explain()` must not print a plan that would be wrong to run."""
        with pytest.raises(UnsupportedFeatureError):
            explain_text(spark.table("fx.mor"))

    def test_an_ordinary_table_is_unaffected(self, spark: SparkSession) -> None:
        assert len(scan_of(spark.table("fx.plain")).groups) == 1
...
class TestUnbindablePlansStillRun:
    def test_an_aggregate_keeps_its_column_name_through_the_optimizer(
        ...
        df = spark.sql("SELECT vendor, count(*) AS n FROM fx.plain GROUP BY vendor")
        assert df.columns == ["vendor", "n"]
        assert sum(row["n"] for row in df.collect()) == 5

    def test_a_nested_table_reads_unchanged(self, spark: SparkSession) -> None:
        """Struct, list and map columns bind as real DuckDB types, so `SELECT *`
        expansion has to survive them."""
        rows = spark.table("fx.nested").collect()
        assert len(rows) == 2
        assert rows[0]["person"]["name"] == "ada"

    @pytest.mark.parametrize("reference", ["fx.plain", "fx.partitioned", "fx.wide"])
    def test_every_fixture_still_reads_end_to_end(
        self, spark: SparkSession, reference: str
    ) -> None:
        assert spark.table(reference).count() > 0
