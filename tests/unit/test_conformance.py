"""The Spark-vs-DuckDB conformance rules of PLAN.md 3.5, at the tree level.

P5 makes Spark the spec, so each rule here is a claim about what DuckDB would
...
def conform(sql: str, *, ansi_mode: bool = False) -> exp.Expression:
    """Parse as Spark and conform -- the real pipeline order, stopping at the tree."""
    parsed = as_expression(sqlglot.parse_one(sql, read="spark"))
    return apply_spark_semantics(parsed, ansi_mode=ansi_mode)


def spark_sql(sql: str, *, ansi_mode: bool = False) -> str:
...
class TestNullOrdering:
    """Spark: ASC nulls first, DESC nulls last. DuckDB 1.5: nulls last for both.
    ...
    def test_ascending_says_so_in_the_generated_sql(self) -> None:
        ...
        assert "NULLS FIRST" in spark_sql("SELECT x FROM t ORDER BY x")
    ...
    def test_an_explicit_choice_is_left_alone(self) -> None:
        """The user asking for the non-Spark order must get the non-Spark order."""
...
class TestCasting:
    """Spark's default (non-ANSI) cast yields NULL on failure; DuckDB's raises.

    The two surfaces arrive spelled differently -- sqlglot's Spark dialect already
    ...
    def test_a_spark_cast_is_lenient_by_default(self) -> None:
        assert "TRY_CAST" in spark_sql("SELECT CAST(x AS INT) FROM t")
    ...
    def test_a_dataframe_style_cast_is_lenient_too(self) -> None:
        ...
        out = apply_spark_semantics(plan).sql(dialect="duckdb")
        assert "TRY_CAST" in out

    def test_ansi_mode_makes_a_cast_strict(self) -> None:
        out = spark_sql("SELECT CAST(x AS INT) FROM t", ansi_mode=True)
        assert "TRY_CAST" not in out
        assert "CAST" in out

    def test_an_explicit_try_cast_stays_lenient_even_under_ansi(self) -> None:
        """`try_cast(...)` means "be lenient because I asked", not "because Spark is",
        so `ansi_mode` has no business changing it."""
        assert "TRY_CAST" in spark_sql("SELECT TRY_CAST(x AS INT) FROM t", ansi_mode=True)

    def test_a_nested_cast_is_covered(self) -> None:
        out = spark_sql("SELECT CAST(CAST(x AS STRING) AS INT) FROM t")
        assert out.count("TRY_CAST") == 2

    def test_the_target_type_survives(self) -> None:
        assert "DECIMAL(10, 2)" in spark_sql("SELECT CAST(x AS DECIMAL(10,2)) FROM t")
...
class TestAlreadyMatching:
    ...
    def test_division_by_zero_is_guarded_by_the_spark_parser(self) -> None:
        """Spark returns NULL for `1/0`; DuckDB returns `inf`. sqlglot's Spark dialect
        already emits the `NULLIF` guard, so no rule of ours is needed."""
        assert "NULLIF" in spark_sql("SELECT 1 / 0")

    def test_null_safe_equality_translates(self) -> None:
        assert "IS NOT DISTINCT FROM" in spark_sql("SELECT a <=> b FROM t")

    def test_modulo_by_zero_needs_no_guard(self) -> None:
        """DuckDB already returns NULL for `x % 0`, as Spark does."""
        assert "NULLIF" not in spark_sql("SELECT x % 0 FROM t")


class TestPassThrough:
    def test_a_plan_with_nothing_to_fix_is_unchanged(self) -> None:
        before = "SELECT a, b FROM t WHERE a > 1"
        assert spark_sql(before) == sqlglot.parse_one(before, read="spark").sql(dialect="duckdb")
